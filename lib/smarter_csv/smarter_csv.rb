module SmarterCSV
    ...
    def SmarterCSV.process(filename, options={}, &block)
        default_options = {:col_sep => ',' , :row_sep => $/ , :quote_char => '"',
                           ...
                           :comment_regexp => /^#/, :chunk_size => nil , :key_mapping_hash => nil , :downcase_header => true, :strings_as_keys => false, :line_limit => 0
        }
        ...
        old_row_sep = $/
            begin
                ...
                loop do
                    break if f.eof?    # we can't use f.readlines() here, because this would read the whole file into memory at once, and eof => true
                    break if result.size >= options[:line_limit] unless options[:line_limit].nil?
